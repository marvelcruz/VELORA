...
const stage=document.querySelector('#home'), shirt=document.querySelector('#shirt'), wrap=document.querySelector('#productWrap');
...
function go(next){if(locked||next===current)return;locked=true;wrap.classList.add('exiting-down');setTimeout(()=>{current=(next+products.length)%products.length;render();wrap.classList.remove('exiting-down');wrap.classList.add('entering-top');requestAnimationFrame(()=>requestAnimationFrame(()=>{wrap.classList.remove('entering-top');setTimeout(()=>locked=false,500)}))},360)}
...
document.querySelectorAll('.nav nav a').forEach(link=>link.addEventListener('click',()=>{document.querySelectorAll('.nav nav a').forEach(x=>x.classList.remove('active'));link.classList.add('active')}));
render();
